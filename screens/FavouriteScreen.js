...
const FavouriteScreen = () => {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/${user._id}/favoriteRestaurants`)
      .then(response => {
  ...
  return (
    <ScrollView>
      ...
      <TouchableOpacity key={restaurant._id}>
        {/* <Text>{user._id}</Text> */}
        <View className="m-2 mt-2 p-2 flex-row justify-between rounded-xl">
          ...
          <View className="w-4/6 right-4">
            ...
            <Text className="text-[#E34B40] font-semibold mt-1 text-base">Đặt bàn giữ chỗ</Text>
